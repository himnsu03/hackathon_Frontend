...
export const AdminProblemStatementsPage = () => {
  ...
  const handleOpenAdd = () => {
    setEditingPs(null);
    setForm({ title: '', description: '', category: 'IoT & Smart Cities', rules: '' });
    setShowModal(true);
  };
  ...
  const handleOpenEdit = (ps) => {
    ...
    setForm({
      ...
      category: ps.category || 'IoT & Smart Cities',
      rules: ps.rules || '',
    });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingPs) {
        await adminApi.updateProblemStatement(editingPs.id, form);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      {/* Main Table Card */}
      <Card title="Active Hackathon Track Problems" subtitle="Showing all registered challenge statements">
        {loading ? (
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          icon={Trash2}
                          onClick={() => handleDeactivate(ps.id, ps.title)}
          ...
          <Card className="max-w-xl w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <Input
                ...
                required
                value={form.title}
                onChange={(e) => setFormData ? setForm({ ...form, title: e.target.value }) : setForm({ ...form, title: e.target.value })}
              ...
              <TextArea
                label="Track Rules & Constraints (Optional)"
                rows={3}
                value={form.rules}
                onChange={(e) => setForm({ ...form, rules: e.target.value })}
                placeholder="Technical architecture or library constraints..."
              />
