...
import { FileText, Loader2, Filter, ChevronRight, Star, FileSpreadsheet, Download } from 'lucide-react';
...
export const EvaluatorSynopsisListPage = () => {
  ...
  const handleExport = () => {
    if (filteredSynopses.length === 0) {
      toast.warning('No synopsis data available to export with current filters.');
      return;
    }

    const excelData = filteredSynopses.map((s, idx) => ({
      'S.No': idx + 1,
      ...
      'Problem Statement': s.problemStatementTitle || s.problemStatementRef || 'N/A',
      'Status': s.status || 'PENDING',
      'AI Score': s.aiOverallScore != null ? s.aiOverallScore : 'N/A',
      'Evaluator Score': s.overallScore != null ? s.overallScore : (s.averageScore != null ? s.averageScore : 'N/A'),
      'Submitted Date': (s.submittedAt || s.createdAt) ? new Date(s.submittedAt || s.createdAt).toLocaleString() : 'N/A',
    }));

    const dateStr = new Date().toISOString().slice(0, 10);
    const fileName = `Synopsis_Submissions_${statusFilter}_${dateStr}.xlsx`;
    exportToExcel(excelData, fileName, 'Synopsis List');
    toast.success(`Exported ${excelData.length} synopsis records to Excel!`);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        ...
        >
          Export Excel ({filteredSynopses.length})
        </Button>
      </div>
