...
export const SynopsisSubmissionPage = () => {
  ...
  useEffect(() => {
    if (selectedProblemId) {
      hackathonApi.getPublicSynopsisAiCriteria(1, selectedProblemId)
        .then((data) => {
                ...
                {problemStatements.map((ps) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-xl border text-left transition-all flex flex-col justify-between relative group ${isSelected
                        ? 'bg-gradient-to-b from-orange-950/40 via-slate-900 to-slate-950 border-orange-500 text-slate-100 ring-2 ring-orange-500/40 shadow-xl shadow-orange-950/30'
                        : 'bg-slate-950/80 border-slate-800 text-slate-400 hover:border-slate-700 hover:bg-slate-900/60 hover:text-slate-200'
                        }`}
                    >
                      {isSelected && (
                        <span className="absolute top-3.5 right-3.5 w-5 h-5 bg-orange-500 text-slate-950 rounded-full flex items-center justify-center shadow-md">
                      ...
                      <div className="space-y-2 pr-6">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-[10px] font-mono text-orange-400 font-bold uppercase tracking-wider bg-orange-500/10 px-2 py-0.5 rounded border border-orange-500/30">
                            {ps.id}
                          </span>
                          {ps.category && (
                            <span className="text-[10px] font-mono text-slate-400 bg-slate-900 px-2 py-0.5 rounded border border-slate-800 truncate max-w-[140px]">
                              {ps.category}
                            </span>
                          )}
                        </div>

                        <h4 className="text-sm font-extrabold text-slate-100 group-hover:text-orange-300 transition-colors line-clamp-2">
                          {ps.title}
                        </h4>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-4 pt-3 border-t border-slate-800/80 w-full">
                        {ps.requirements?.length > 0 && (
                          <span className="text-[10px] font-mono text-indigo-400 bg-indigo-950/60 px-2 py-0.5 rounded border border-indigo-800/40 font-semibold">
                            {ps.requirements.length} Reqs
                          </span>
                        )}
                        {ps.deliverables?.length > 0 && (
                          <span className="text-[10px] font-mono text-amber-400 bg-amber-950/60 px-2 py-0.5 rounded border border-amber-800/40 font-semibold">
                        ...
                        <span className="text-[10px] font-semibold text-orange-400/90 ml-auto group-hover:text-orange-300">
